...
On brainlife.io, we have many options for performing tractography! For this tutorial, we will focus on Probabilistic tractography, using the combined **anatomically-constrained tractography (ACT)** app, which restricts streamline representations to those that are biologically-plausible (i.e. not crossing CSF or gray matter)[mrtrix3 - WMC Anatomically Constrained Tractography (ACT) ](https://brainlife.io/app/5e9dced9f1745d6994f692c0) app! Within this app, both the CSD and DTI models will be fit to the diffusion data and returned as outputs!

## Brainlife Mobile Application Tutorial

The **Brainlife Mobile** application is the official mobile client for the brainlife.io platform. It is designed to let you monitor and manage your projects, datasets, apps, and publications on the go, bringing the power of brainlife.io's neuroimaging analysis directly to your mobile device.
...
2. Tap the sign-in button. The app will open the official Brainlife web auth flow (`https://brainlife.io/auth/#!/signin?app=mobile`) in a secure WebView container.
3. Sign in using your standard Brainlife credentials.
4. Once authenticated, the WebView securely passes your JSON Web Token (JWT) back to the mobile client, which stores it securely using on-device encrypted storage (`Expo SecureStore`). You will remain signed in across app restarts.

---

### B. Create a Project

To begin managing and processing datasets in the mobile app, you need to create a project space.

1. Open the sidebar navigation menu in the top-left corner.
2. Select **Projects** to navigate to the project browser.
3. Press the **+** (plus) floating action button located at the bottom-right corner of the screen.
...
2. Press either **Copy from another project** or **Import public dataset**.
...
4. To copy project data:
    ...
    * Press the **Copy to project** action button.
...
4. On the **Select App** tab, the application catalog will automatically recommend optimal pipelines and applications based on the datatype and input requirements of your selected dataset.
...
4. Browse or search for your desired application in the App Store catalog, define the inputs, and submit the process.
...
2. Follow the 4-step stepper timeline to construct your pipeline:
    * **Step 1: Select App**: Choose the Brainlife application to run.
...
* **Deep Linking**: Tap any notification card to jump directly to the relevant project page or the process timeline on your mobile client to debug failures or review successful outputs.

---

### Technical Stack Overview

For developers interested in contributing or extending the mobile client, the technical architecture consists of:
